function Invoke-TestHarness
{
    ...
    Write-Verbose -Message 'Commencing all WSManDsc tests'

    $repoDir = Join-Path -Path $PSScriptRoot -ChildPath '..\' -Resolve

    $testCoverageFiles = @()
    Get-ChildItem -Path "$repoDir\modules\WSManDsc\DSCResources\**\*.psm1" -Recurse | ForEach-Object {
    ...
    Import-Module -Name "$repoDir\modules\WSManDsc\WSManDsc.psd1"
    $testsToRun = @()
